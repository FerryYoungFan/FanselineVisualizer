...
if __name__ == '__main__':
    ...
    fb = FanBlender()
    fb.setFilePath(image_path=r"./Input/头 像.png",
                   sound_path=r"./Input/测 试2.mp3",
                   logo_path=r"./Input/logo.png")
    fb.setOutputPath(output_path=r"./Output",
                     filename="test.mp4")
    fb.setText(text="测试测试", font="./Source/font.otf")
    fb.setSpec(bins=80, lower=20, upper=1500, color=fb.color_dic["Gradient: Green - Blue"],scalar=1.0)
    fb.setVideoInfo(width=520, height=520, fps=30, br_Mbps=5)
    fb.previewBackground()
    fb.setAudioInfo(normal=False, br_kbps=192)
    print(fb.getSpec())
    print(fb.getVideoInfo())
    #fb.runBlending()
